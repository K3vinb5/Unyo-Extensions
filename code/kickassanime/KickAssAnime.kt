package extensions.en.kickassanime


import java.util.Base64
import extensions.en.kickassanime.dto.AnimeInfoDto
import extensions.en.kickassanime.dto.EpisodeResponseDto
import extensions.en.kickassanime.dto.LanguagesDto
import extensions.en.kickassanime.dto.PopularItemDto
import extensions.en.kickassanime.dto.PopularResponseDto
import extensions.en.kickassanime.dto.RecentsResponseDto
import extensions.en.kickassanime.dto.SearchResponseDto
import extensions.en.kickassanime.dto.ServersDto
import extensions.en.kickassanime.extractors.KickAssAnimeExtractor
...
class KickAssAnime : ConfigurableAnimeSource, AnimeHttpSource() {
    ...
    override val baseUrl = preferences.get(PREF_DOMAIN_KEY, PREF_DOMAIN_DEFAULT)
    ...
    override suspend fun getEpisodeList(anime: SAnime): List<SEpisode> {
        ...
        val prefLang = preferences.get(PREF_AUDIO_LANG_KEY, PREF_AUDIO_LANG_DEFAULT)!!
    ...
    private fun searchAnimeRequest(page: Int, query: String, filters: KickAssAnimeFilters.FilterSearchParams?): Request {
        ...
        if (filters != null) if (filters.subPage.isNotBlank()) return GET("$baseUrl/api/${filters.subPage}?page=$page", headers = newHeaders)

        val encodedFilters = if (filters != null) if (filters.filters == "{}") "" else Base64.getEncoder().encodeToString(filters.filters.encodeToByteArray()) else ""
    ...
    override suspend fun getSearchAnime(page: Int, query: String, filters: AnimeFilterList): AnimesPage {
        return if (query.startsWith(PREFIX_SEARCH)) { // URL intent handler
            ...
            return client.newCall(searchAnimeRequest(page, query, null))
                .awaitSuccess()
                .let { response ->
...
//    }
    ...
    companion object {
        ...
        private const val PREF_DOMAIN_DEFAULT = "https://www1.kickassanime.mx/"
        ...
        private val PREF_HOSTER_DEFAULT = SERVERS.joinToString(",")
    }

    override fun setupPreferenceScreen(screen: Preferences) {
        preferences.putBoolean(PREF_USE_ENGLISH_KEY, PREF_USE_ENGLISH_DEFAULT)
        preferences.put(PREF_QUALITY_KEY, PREF_QUALITY_DEFAULT)
        preferences.put(PREF_AUDIO_LANG_KEY, PREF_AUDIO_LANG_DEFAULT)
        preferences.put(PREF_SERVER_KEY, PREF_SERVER_DEFAULT)
        preferences.put(PREF_DOMAIN_KEY, PREF_DOMAIN_DEFAULT)
