package extensions.all.torrentio
...
import extensions.all.torrentio.dto.EpisodeList
import extensions.all.torrentio.dto.GetPopularTitlesResponse
import extensions.all.torrentio.dto.GetUrlTitleDetailsResponse
import extensions.all.torrentio.dto.StreamDataTorrent
...
//    private val handler by lazy { Handler(Looper.getMainLooper()) }
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        val country = preferences.get(PREF_REGION_KEY, PREF_REGION_DEFAULT)
        val language = preferences.get(PREF_JW_LANG_KEY, PREF_JW_LANG_DEFAULT)
        val perPage = 40
        val packages = ""
        val year = 0
        val objectTypes = ""
    ...
    override fun videoListRequest(episode: SEpisode): Request {
        val mainURL = buildString {
            ...
            appendQueryParam("providers", preferences.get(PREF_PROVIDER_KEY, PREF_PROVIDERS_DEFAULT.joinToString("@")).split("@").toSet())
            appendQueryParam("language", preferences.get(PREF_LANG_KEY, PREF_LANG_DEFAULT.joinToString("@")).split("@").toSet())
            appendQueryParam("qualityfilter", preferences.get(PREF_QUALITY_KEY, PREF_QUALITY_DEFAULT.joinToString("@")).split("@").toSet())
...
//                }
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        val videoList = streamList.streams?.map { stream ->
            val urlOrHash =
