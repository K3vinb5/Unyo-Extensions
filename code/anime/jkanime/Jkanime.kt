...
import extensions.es.jkanime.extractors.JkanimeExtractor
...
class Jkanime : ConfigurableAnimeSource, ParsedAnimeHttpSource() {
    ...
    private fun getVideoLinks(document: Document): List<Pair<String, String>> {
        ...
        var jsLinksString: String? = "";
        if (isRemote && jsServer.isNotEmpty()) {
            jsLinksString = client.newCall(GET(jsServer + jsPath)).execute().body?.string().toString()
        } else {
            jsLinksString = scriptServers.substringAfter("var servers = ").substringBefore(";").substringBefore("var")
        }

        val jsLinks = jsLinksString.parseAs<Array<JsLinks>>().map {
            Pair(String(Base64.getDecoder().decode(it.remote)), "${it.lang}".getLang())
        }
    ...
    override fun List<Video>.sort(): List<Video> {
        ...
        val lang = preferences.get(PREF_LANGUAGE_KEY, PREF_LANGUAGE_DEFAULT)!!
